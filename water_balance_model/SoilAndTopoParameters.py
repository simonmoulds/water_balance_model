...
class SoilAndTopoParameters(object):
    ...
    def __init__(self, SoilAndTopoParameters_variable, config_section):
    ...
    def initial(self):
        ...
        # for convenience
        self.var.dz_xy = np.broadcast_to(self.var.dz[:,None], (self.var.nComp, self.var.nCell))
        self.var.dzsum_xy = np.broadcast_to(self.var.dzsum[:,None], (self.var.nComp, self.var.nCell))
    ...
    def readSoil(self):
        ...
        # Layers:
        zlayermid = vos.get_dimension_variable(self.var.soilAndTopoFileNC,'layer')
        ...
        # Compartments:
        zmid = vos.get_dimension_variable(self.var.soilAndTopoFileNC,'compartment')
        ...
        landmask = np.broadcast_to(self.var.landmask, (nlayer, self.var.nLat, self.var.nLon))
        for param in soilParams1:
            d = vos.netcdf2PCRobjCloneWithoutTime(
                self.var.soilAndTopoFileNC,
                param,
                cloneMapFileName=self.var.cloneMap)
            d = d[landmask].reshape(nlayer,self.var.nCell)
    ...
    def compute_capillary_rise_parameters(self):
        ...
        self.var.aCR = np.zeros((self.var.nLayer, self.var.nCell))
        self.var.bCR = np.zeros((self.var.nLayer, self.var.nCell))
