...
class RootZoneWater(object):
    ...
    def dynamic(self):
        ...
        Wr = np.sum(Wr_comp, axis=2)
        Wr[Wr < 0] = 0
        WrS = np.sum(WrS_comp, axis=2)
        WrFC = np.sum(WrFC_comp, axis=2)
        WrWP = np.sum(WrWP_comp, axis=2)
        WrDry = np.sum(WrDry_comp, axis=2)
        # print 'Wr:',Wr.shape
        # print WrS.shape
        # print WrFC.shape
        # print WrWP.shape
...
class AQRootZoneWater(RootZoneWater):
    ...
    def dynamic(self):
        ...
        Aer_comp = np.broadcast_to(self.var.Aer[:,:,None,:], (self.var.nFarm, self.var.nLC, self.var.nComp, self.var.nCell))
        WrAer_comp = self.var.RootFact * 1000 * (self.var.th_s_comp - (Aer_comp / 100)) * self.var.dz_xy
