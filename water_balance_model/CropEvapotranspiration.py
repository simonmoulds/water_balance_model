...
class CropEvapotranspiration(object):
    ...
    transpiration
    """
    def __init__(self, CropEvapotranspiration_variable):
        self.var = CropEvapotranspiration_variable
    ...
    def dynamic(self):
        ...
        # Kc = np.zeros((self.var.nLC, self.var.nCell))
        ...
        # self.var.ETpot = np.broadcast_to(
        #     self.var.referencePotET,
        #     (self.var.nLC, self.var.nCell)) * Kc
        ...
        # self.var.ETact = np.clip(self.var.ETact, 0, np.sum(max_available_water, axis=2))
        
        # # Extract water proportionally
        # # TODO: this should be improved!!! (how does PCR-GLOBWB do this?)        
        # f = (max_available_water / np.broadcast_to(np.sum(max_available_water, axis=2)[:,:,None,:], max_available_water.shape))
        ...
        # # Accumulate ETpot and ETact (necessary for yield computation)
