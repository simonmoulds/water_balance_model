...
class PreIrrigation(object):
    ...
    def dynamic(self):
        ...
        # arr_ones = np.ones((self.var.nLC, self.var.nCell))[:,None,:]
        # dz = (self.var.dz[None,:,None] * arr_ones)
        # dzsum = (self.var.dzsum[None,:,None] * arr_ones)
        ...
        rootdepth = np.broadcast_to(rootdepth[:,:,None,:], (self.var.nFarm, self.var.nLC, self.var.nComp, self.var.nCell))
        netirrsmt = np.broadcast_to(self.var.NetIrrSMT[:,:,None,:], (self.var.nFarm, self.var.nLC, self.var.nComp, self.var.nCell))
        ...
        cond0 = np.broadcast_to(cond0[:,:,None,:], (self.var.nFarm, self.var.nLC, self.var.nComp, self.var.nCell))
        
        # cond1 = ((dzsum - dz) < np.broadcast_to(rootdepth[:,None,:], self.var.th.shape)) & (self.var.th < thCrit)
        cond1 = (cond0
                 & ((self.var.dzsum_xy - self.var.dz_xy) < rootdepth)
                 & (self.var.th < thCrit))
        ...
        self.var.th[cond1] = thCrit[cond1]
        # self.var.PreIrr[np.logical_not(cond0)] = 0
